...
class ServerProcess:
    ...
    def __init__(self):
        ...
        if "N_THREADS" in os.environ:
            self.n_threads = int(os.environ["N_THREADS"])
        elif self.n_threads is None:
            self.n_threads = 16
    ...
    def start(self, timeout_seconds: int = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        # wait for server to start by reading the log, with HTTP polling fallback
        start_time = time.time()
        has_log = self.log_path is not None and os.path.exists(self.log_path)
        ready = False
        while time.time() - start_time < timeout_seconds:
            # Primary: read the log file for "server is listening on"
            if has_log:
                with open(self.log_path, encoding="utf-8", errors="replace") as f:
                    if "server is listening on" in f.read():
                        ready = True
                        break
        ...
        # Confirm server is truly ready with one health check
        for _ in range(5):
            try:
                r = self.make_request(
                    "GET",
                ...
                pass
            time.sleep(0.5)
        # Server logged "listening" but healthcheck failed -- still mark ready,
        # the next test request will fail fast if truly unreachable
        self.ready = True
