...
#define LLAMA_STATE_SEQ_MAGIC_COMPRESSED (LLAMA_FILE_MAGIC_GGSQ ^ 0x20242025u)

// Compression statistics (zstd only, populated when --kv-cache-compress is active)
LLAMA_API void llama_kv_cache_compression_stats_reset(void);
LLAMA_API void llama_kv_cache_compression_stats_get(int64_t *  total_compress_us,
...
// Dictionary learning mode for KV cache compression
enum llama_kv_cache_compress_learn_type {
    LLAMA_KV_CACHE_COMPRESS_LEARN_NONE         = 0,
    LLAMA_KV_CACHE_COMPRESS_LEARN_SAMPLE_FIRST = 1,
    LLAMA_KV_CACHE_COMPRESS_LEARN_INCREMENTAL  = 2,
    LLAMA_KV_CACHE_COMPRESS_LEARN_CONTINUOUS   = 3,
};
...
struct llama_context_params {
    ...
    int32_t compress_kv_cache;  // 0 = disabled, -22..22 = zstd level (negative = fast mode)

    enum llama_kv_cache_compress_learn_type compress_kv_cache_learn;
