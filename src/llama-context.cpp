...
llama_context::llama_context(const llama_model & model, llama_context_params params) :
    ...
    compress_kv_cache       = params.compress_kv_cache;
    compress_kv_cache_learn = params.compress_kv_cache_learn;
#ifdef LLAMA_HAS_ZSTD
    ...
    if (model.arch == LLM_ARCH_EAGLE3) {
        if (model.tok_embd == nullptr || model.output == nullptr) {
            if (params.ctx_other == nullptr) {
                throw std::runtime_error(
                    "EAGLE3 requires ctx_other to be set (this warning is normal during memory fitting)");
            }
            cparams.ctx_other = params.ctx_other;
        }
    }
...
//
// zstd-compressed I/O wrappers for KV cache disk
//

#ifdef LLAMA_HAS_ZSTD

// Global compression statistics (exposed via llama_kv_cache_compression_stats)
struct {
...
// Streaming zstd compressor that wraps a llama_file.
// Writes a compressed frame using ZSTD_compressStream2.
...
// zstd decompressor that wraps a llama_file.
// Pre-reads all remaining compressed data, then decompresses on demand.
class llama_io_read_zstd : public llama_io_read_i {
  public:
    ...
    ~llama_io_read_zstd() override {
        ZSTD_freeDCtx(dctx);
        ZSTD_freeDDict(ddct);
        // Record decompression metrics. If we had a valid dctx, report the time.
        // t_start_us is set in the constructor if dctx was successfully created.
        // We report even partially-read streams.
...
// Trains a zstd dictionary from saved KV cache data using ZDICT_optimizeTrainFromBuffer_fastCover.
class llama_kv_dict {
  public:
    ...
    bool train(int level = 3) {
        ...
        LLAMA_LOG_INFO("%s: dictionary trained, size=%zu bytes from %zu samples (total %zu bytes)\n", __func__,
                       dict_size, samples.size(), all_data.size());
...
size_t llama_context::state_seq_load_file(llama_seq_id  seq_id,
    ...
    bool     compressed = false;
    {
        ...
        if (magic == LLAMA_STATE_SEQ_MAGIC_COMPRESSED) {
            compressed = true;
    ...
    // restore the context state (compressed or uncompressed)
    {
        if (compressed) {
#ifdef LLAMA_HAS_ZSTD
...
size_t llama_context::state_seq_save_file(llama_seq_id        seq_id,
    ...
    // uncompressed write (original path)
...
llama_context_params llama_context_default_params() {
    llama_context_params result = {
        ...
        /*.compress_kv_cache           =*/0,
        /*.compress_kv_cache_learn     =*/LLAMA_KV_CACHE_COMPRESS_LEARN_NONE,
