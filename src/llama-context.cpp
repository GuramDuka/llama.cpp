...
size_t llama_context::state_seq_load_file(llama_seq_id  seq_id,
    ...
    // restore the context state
    {
        ...
        llama_io_read_file io(&file);
        size_t             io_before = io.n_bytes();
        const size_t       nread     = state_seq_read_data(io, seq_id, 0);
        size_t             io_after  = io.n_bytes();
        LLAMA_LOG_ERROR("%s: state_size=%zu, io_before=%zu, io_after=%zu, nread=%zu, seq_id=%d\n", __func__, state_size,
                        io_before, io_after, nread, seq_id);
        if (!nread) {
