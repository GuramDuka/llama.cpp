...
struct common_speculative_impl {
    ...
    // (optional) get/set pending_h embedding (used by MTP for carry-over between process() calls)
    virtual size_t get_pending_h(llama_seq_id /*seq_id*/, float * /*h*/) const { return 0; }

    virtual void set_pending_h(llama_seq_id /*seq_id*/, const float * /*h*/, size_t /*n_embd*/) {}
...
struct common_speculative_impl_draft_eagle3 : public common_speculative_impl {
    ...
    common_speculative_impl_draft_eagle3(const common_params_speculative & params, uint32_t n_seq) :
        ...
        LOG_INF("%s: - n_max=%d, n_min=%d, p_min=%f, backend_sampling=%d\n", __func__, params.draft.n_max,
                params.draft.n_min, params.draft.p_min, (int) params.draft.backend_sampling);
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        if (pos_max < N - 2) {
            LOG_WRN(
                "%s: ctx_dft pos_max=%d < N-2=%d — process() did not run on every prefill ubatch. "
                "Drafts may degrade.\n",
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        for (int32_t i = 0; i < n_tokens; i += n_ubatch_dft) {
            ...
            std::memcpy(g_embd_buf.data() + (size_t) i * n_embd_dec, g_embd_chunk,
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (pending_pos >= 0 && pending_pos + 1 == batch_in.pos[beg]) {
                ...
                if (pending_pos > dft_pos_max) {
                    ...
                    std::memcpy(batch.embd + (size_t) (batch.n_tokens - 1) * n_embd_dec, pending_g_last[seq_id].data(),
                                row_bytes);
                }
            }
            ...
            for (int32_t k = beg; k < end; ++k) {
                ...
                std::memcpy(batch.embd + (size_t) (batch.n_tokens - 1) * n_embd_dec, g_embd + (size_t) k * n_embd_dec,
                            row_bytes);
            }
        ...
        if (batch.n_tokens > 0) {
            ...
            if (rc != 0) {
                LOG_ERR("%s: llama_decode(ctx_dft) failed rc=%d (n_tokens=%d, ubatch_pos[0]=%d)\n", __func__, rc,
                        (int) batch.n_tokens, (int) batch_in.pos[0]);
                return false;
            }
        }
    ...
    void accept(llama_seq_id seq_id, uint16_t n_accepted, bool /*is_other*/) override {
        ...
        std::memcpy(pending_g_last[seq_id].data(), verify_g[seq_id].data() + (size_t) i_g * n_embd_dec,
    ...
    void set_state(llama_seq_id seq_id, const std::vector<uint8_t> & data) override {
        ...
        std::memcpy(pending_g_last[seq_id].data(), data.data() + sizeof(llama_pos),
                    (size_t) n_embd_dec * sizeof(float));
    }
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    void begin(llama_seq_id seq_id, const llama_tokens & prompt) override {
        ...
        // Reset pending_h to avoid carrying stale embeddings from a previous request.
        // This is important because begin() is called after all process() calls for the
        // current prompt, and without a reset the NEXT request's first process() call
        // would use embeddings left over from the previous request's generation.
        if (seq_id >= 0 && (size_t) seq_id < pending_h.size()) {
            std::fill(pending_h[seq_id].begin(), pending_h[seq_id].end(), 0.0f);
        }
    }

    size_t get_pending_h(llama_seq_id seq_id, float * h) const override {
        if (seq_id < 0 || (size_t) seq_id >= pending_h.size()) {
            return 0;
        }
    ...
    void set_pending_h(llama_seq_id seq_id, const float * h, size_t n_embd) override {
        if (seq_id < 0 || (size_t) seq_id >= pending_h.size()) {
            return;
        }
        if (h == nullptr || n_embd == 0) {
            return;
        }
        pending_h[seq_id].assign(h, h + n_embd);
    }
    ...
    bool need_embd_nextn() const override { return true; }
};
...
struct common_speculative_impl_ngram_map_k : public common_speculative_impl {
    ...
    common_speculative_impl_ngram_map_k(const common_ngram_map & config, uint32_t n_seq) :
        common_speculative_impl(
            config.key_only ? COMMON_SPECULATIVE_TYPE_NGRAM_MAP_K : COMMON_SPECULATIVE_TYPE_NGRAM_MAP_K4V,
            n_seq) {
...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    std::vector<common_speculative_config> configs = {};  // list of speculative configs to try
    {
        ...
        bool has_draft_eagle3 =
            (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3)) && params.draft.ctx_dft != nullptr;
        bool has_draft_mtp =
            (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_MTP)) && params.draft.ctx_dft != nullptr;
...
size_t common_speculative_get_pending_h(common_speculative * spec, llama_seq_id seq_id, float * h) {
    ...
    for (auto & impl : spec->impls) {
        size_t n = impl->get_pending_h(seq_id, h);
        if (n > 0) {
            return n;
        }
    }
...
void common_speculative_set_pending_h(common_speculative * spec, llama_seq_id seq_id, const float * h, size_t n_embd) {
    ...
    for (auto & impl : spec->impls) {
        impl->set_pending_h(seq_id, h, n_embd);
    }
}
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        if (impl->n_call_accept > 0) {
            const double       mean = 1.0 + (double) impl->n_acc_tokens / (double) impl->n_call_accept;
        ...
        LOG_INF(
            ...
            str_stats.c_str(), str_perf.c_str());
    }
}
