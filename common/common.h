...
inline bool common_grammar_needs_prefill(const common_grammar & g) {
    return g.type == COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT || g.type == COMMON_GRAMMAR_TYPE_TOOL_CALLS;
}
...
struct common_params_speculative {
    ...
    uint32_t need_n_rs_seq() const {
        bool needs_rs_seq =
            std::any_of(types.begin(), types.end(), [&](auto t) { return t == COMMON_SPECULATIVE_TYPE_DRAFT_MTP; });
...
template <> inline std::vector<std::string> string_split<std::string>(const std::string & str, char delim) {
