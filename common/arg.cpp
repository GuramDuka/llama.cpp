...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    if (params.model.path.empty() && ctx_arg.ex != LLAMA_EXAMPLE_SERVER && !skip_model_download && !params.usage &&
        !params.completion) {
...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "--ui-config" }, "JSON", "JSON that provides default UI settings (overrides UI defaults)",
                       [](common_params & params, const std::string & value) {
                           params.ui_config_json    = value;
                           params.webui_config_json = value;
                       })
    ...
    add_opt(common_arg({ "--webui-config-file" }, "PATH",
                       "[DEPRECATED: use --ui-config-file] JSON file that provides default WebUI settings (overrides "
                       "WebUI defaults)",
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--ui-config-file" }, "PATH",
    ...
    add_opt(
        common_arg({ "--webui-mcp-proxy" }, { "--no-webui-mcp-proxy" },
            ...
            .set_examples({ LLAMA_EXAMPLE_SERVER })
            .set_env("LLAMA_ARG_WEBUI_MCP_PROXY"));

    add_opt(common_arg({ "--ui-mcp-proxy" }, { "--no-ui-mcp-proxy" },
    ...
    add_opt(common_arg({ "--webui" }, { "--no-webui" }, "[DEPRECATED: use --ui/--no-ui] whether to enable the Web UI",
                       [](common_params & params, bool value) {
                           params.ui    = value;
                           params.webui = value;
                       })
