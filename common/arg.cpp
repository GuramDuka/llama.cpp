...
bool common_params_handle_models(common_params &                            params,
                                 llama_example                              curr_ex,
                                 const common_params_handle_models_params & handle_params) {
    const bool spec_type_draft_mtp = std::find(params.speculative.types.begin(), params.speculative.types.end(),
    ...
    try {
        ...
        if (spec_type_draft_mtp && res.found_mtp && params.speculative.draft.mparams.path.empty() &&
            params.speculative.draft.mparams.hf_repo.empty() && params.speculative.draft.mparams.url.empty()) {
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    auto parse_cli_args = [&]() {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (!seen_args.insert(arg).second) {
                ...
                if (!skip) {
                    LOG_WRN(
                        "DEPRECATED: argument '%s' specified multiple times, use comma-separated values instead (only "
    ...
    if (!skip_model_download) {
        ...
        if (params.model.path.empty() && !params.usage && !params.completion) {
...
common_params_context common_params_parser_init(common_params & params,
    ...
    add_opt(common_arg({ "--verbose-prompt" },
                ...
                .set_examples(
                    { LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_EMBEDDING, LLAMA_EXAMPLE_RETRIEVAL }));
    ...
    add_opt(common_arg({ "-ctxcp", "--ctx-checkpoints", "--swa-checkpoints" }, "N",
    ...
    add_opt(
        common_arg({ "-cms", "--checkpoint-min-step" }, "N",
                   string_format("minimum spacing between context checkpoints in tokens (default: %d, 0 = no minimum)",
                                 params.checkpoint_min_step),
                   [](common_params & params, int value) {
                       if (value < 0) {
    ...
    add_opt(common_arg({ "--cache-idle-slots" }, { "--no-cache-idle-slots" },
                ...
                .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "--kv-cache-auto" }, { "--no-kv-cache-auto" },
                       "enable automatic KV cache save/restore to disk (default: disabled)",
                       [](common_params & params, bool value) { params.kv_cache_auto = value; })
                .set_env("LLAMA_ARG_KV_CACHE_AUTO")
                .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg({ "-cgs", "--max-cache-size-gb" }, "N",
                ...
                .set_env("LLAMA_ARG_MAX_CACHE_SIZE_GB")
                .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(common_arg(
                { "-cttl", "--cache-ttl" }, "N",
                string_format("delete cache files older than this many seconds (0 = disabled, default: %" PRId64 "s)",
    ...
    add_opt(common_arg({ "--ignore-eos" },
                ...
                .set_sampling());
    add_opt(common_arg({ "--temp", "--temperature" }, "N",
    ...
    add_opt(common_arg(
                ...
                .set_sampling());
    add_opt(
        common_arg({ "--typical", "--typical-p" }, "N",
    ...
    add_opt(common_arg({ "--grammar" }, "GRAMMAR",
                ...
                .set_sampling());
    add_opt(common_arg({ "--grammar-file" }, "FNAME", "file to read grammar from",
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(
        common_arg({ "-j", "--json-schema" }, "SCHEMA",
                   ...
                   [](common_params & params, const std::string & value) {
                       params.sampling.grammar = { COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT,
                                                   json_schema_to_grammar(json::parse(value)) };
                   })
            .set_sampling());
    add_opt(common_arg(
                ...
                [](common_params & params, const std::string & value) {
                    ...
                    params.sampling.grammar = { COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT,
                                                json_schema_to_grammar(json::parse(schema)) };
                })
                .set_sampling());
    ...
    add_opt(common_arg({ "--image", "--audio", "--video" }, "FILE",
                       "path to an image, audio, or video file. use with multimodal models, use comma-separated values "
                       "for multiple files\n",
                       [](common_params & params, const std::string & value) {
                           for (const auto & item : parse_csv_row(value)) {
    ...
    add_opt(common_arg({ "--mtmd-batch-max-tokens" }, "N",
                       string_format("maximum number of image tokens per batch when encoding images (default: %d)",
                                     params.mtmd_batch_max_tokens),
                       [](common_params & params, int value) { params.mtmd_batch_max_tokens = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "-sm", "--split-mode" }, "{none,layer,row,tensor}",
        ...
        common_arg(
            ...
            [](common_params & params, const std::string & value) {
                if (is_truthy(value)) {
                    ...
                    throw std::runtime_error(string_format("error: unknown value for --fit: '%s'\n", value.c_str()));
                }
            })
            .set_env("LLAMA_ARG_FIT"));
    add_opt(common_arg({ "-fitp", "--fit-print" }, "[on|off]",
                       string_format("print the estimated required memory ('on' or 'off', default: '%s')",
                                     params.fit_params_print ? "on" : "off"),
                       [](common_params & params, const std::string & value) {
                           if (is_truthy(value)) {
                       ...
                       [](common_params & params, const std::string & value) {
                           ...
                           if (split_arg.size() == 1) {
                               std::fill(params.fit_params_target.begin(), params.fit_params_target.end(),
                                         std::stoull(split_arg[0]) * 1024 * 1024);
                               return;
                           }
    ...
    add_opt(common_arg({ "-a", "--alias" }, "STRING", "set model name aliases, comma-separated (to be used by API)",
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--tags" }, "STRING", "set model tags, comma-separated (informational, not used for routing)",
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "-o", "--output", "--output-file" }, "FNAME",
                ...
                .set_examples({ LLAMA_EXAMPLE_IMATRIX, LLAMA_EXAMPLE_CVECTOR_GENERATOR, LLAMA_EXAMPLE_EXPORT_LORA,
                                LLAMA_EXAMPLE_TTS, LLAMA_EXAMPLE_FINETUNE, LLAMA_EXAMPLE_RESULTS,
    ...
    add_opt(common_arg({ "--reuse-port" },
                       string_format("allow multiple sockets to bind to the same port (default: %s)",
                                     params.reuse_port ? "enabled" : "disabled"),
                       [](common_params & params) { params.reuse_port = true; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--ui-config", "--webui-config" }, "JSON",
                       "JSON that provides default UI settings (overrides UI defaults)",
                       [](common_params & params, const std::string & value) { params.ui_config_json = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_UI_CONFIG"));
    add_opt(
        common_arg({ "--ui-config-file", "--webui-config-file" }, "PATH",
                   "JSON file that provides default UI settings (overrides UI defaults)",
                   [](common_params & params, const std::string & value) { params.ui_config_json = read_file(value); })
            .set_examples({ LLAMA_EXAMPLE_SERVER })
            .set_env("LLAMA_ARG_UI_CONFIG_FILE"));
    add_opt(common_arg({ "--ui-mcp-proxy", "--webui-mcp-proxy" }, { "--no-ui-mcp-proxy", "--no-webui-mcp-proxy" },
                       "experimental: whether to enable MCP CORS proxy - do not enable in untrusted environments "
                       "(default: disabled)",
                       [](common_params & params, bool value) { params.ui_mcp_proxy = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_UI_MCP_PROXY"));
    add_opt(common_arg(
                { "--tools" }, "TOOL1,TOOL2,...",
                "experimental: whether to enable built-in tools for AI agents - do not enable in untrusted "
                "environments (default: no tools)\n"
                "specify \"all\" to enable all tools\n"
                "available tools: read_file, file_glob_search, grep_search, exec_shell_command, write_file, edit_file, "
                "apply_diff, get_datetime",
                [](common_params & params, const std::string & value) { params.server_tools = parse_csv_row(value); })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
                .set_env("LLAMA_ARG_TOOLS"));
    add_opt(common_arg({ "-ag", "--agent" }, { "-no-ag", "--no-agent" },
    ...
    add_opt(common_arg({ "--ui", "--webui" }, { "--no-ui", "--no-webui" },
                       string_format("whether to enable the Web UI (default: %s)", params.ui ? "enabled" : "disabled"),
                       [](common_params & params, bool value) { params.ui = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--api-key-file" }, "FNAME",
                       "path to file containing API keys, one per line; lines starting with a hash are treated as "
    ...
    add_opt(common_arg({ "--sse-ping-interval" }, "N",
                       string_format("server SSE ping interval in seconds (-1 = disabled, default: %d)",
                                     params.sse_ping_interval),
                       [](common_params & params, int value) { params.sse_ping_interval = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(
        common_arg(
            ...
            [](common_params & params, const std::string & value) { params.sampling.reasoning_budget_message = value; })
            .set_examples({ LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_CLI })
    ...
    add_opt(common_arg(
                ...
                [](common_params & params, bool value) { params.force_pure_content_parser = value; })
                .set_examples({ LLAMA_EXAMPLE_COMPLETION, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER })
    ...
    add_opt(common_arg({ "--log-prompts-dir" }, "PATH",
                       "Log prompts to directory (only used for debugging, default: disabled)",
                       [](common_params & params, const std::string & value) { params.path_prompts_log_dir = value; })
                .set_examples({ LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    ...
    add_opt(common_arg({ "--log-prefix" }, { "--no-log-prefix" }, "Enable prefix in log messages",
                       [](common_params &, bool value) { common_log_set_prefix(common_log_main(), value); })
                .set_env("LLAMA_ARG_LOG_PREFIX"));
    add_opt(common_arg({ "--log-timestamps" }, { "--no-log-timestamps" }, "Enable timestamps in log messages",
                       [](common_params &, bool value) { common_log_set_timestamps(common_log_main(), value); })
    ...
    add_opt(common_arg({ "--spec-draft-hf", "-hfd", "-hfrd", "--hf-repo-draft" }, "<user>/<model>[:quant]",
    ...
    add_opt(common_arg({ "--spec-draft-threads", "-td", "--threads-draft" }, "N",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-threads-batch", "-tbd", "--threads-batch-draft" }, "N",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-mask", "-Cd", "--cpu-mask-draft" }, "M",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-range", "-Crd", "--cpu-range-draft" }, "lo-hi",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-strict", "--cpu-strict-draft" }, "<0|1>",
                       "Use strict CPU placement for draft model (default: same as --cpu-strict)",
                       [](common_params & params, int value) { params.speculative.draft.cpuparams.strict_cpu = value; })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-prio", "--prio-draft" }, "N",
                       string_format(
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-poll", "--poll-draft" }, "<0|1>",
                       "Use polling to wait for draft model work (default: same as --poll)",
                       [](common_params & params, int value) { params.speculative.draft.cpuparams.poll = value; })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-mask-batch", "-Cbd", "--cpu-mask-batch-draft" }, "M",
                       "Draft model CPU affinity mask. Complements cpu-range-draft (default: same as --cpu-mask)",
                       [](common_params & params, const std::string & mask) {
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-range-batch", "-Crbd", "--cpu-range-batch-draft" }, "lo-hi",
    ...
    add_opt(common_arg(
                ...
                [](common_params & params, int value) { params.speculative.draft.cpuparams_batch.strict_cpu = value; })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-prio-batch", "--prio-batch-draft" }, "N",
                       string_format(
                           "set draft process/thread priority : 0-normal, 1-medium, 2-high, 3-realtime (default: %d)\n",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-poll-batch", "--poll-batch-draft" }, "<0|1>",
                       "Use polling to wait for draft model work (default: --poll-draft)",
                       [](common_params & params, int value) { params.speculative.draft.cpuparams_batch.poll = value; })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg(
                ...
                string_format("KV cache data type for K for the draft model\n"
                              "allowed values: %s\n"
                              "(default: %s)",
                              get_all_kv_cache_types().c_str(), ggml_type_name(params.speculative.draft.cache_type_k)),
                [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
                ...
                string_format("KV cache data type for V for the draft model\n"
                              "allowed values: %s\n"
                              "(default: %s)",
                              get_all_kv_cache_types().c_str(), ggml_type_name(params.speculative.draft.cache_type_v)),
                [](common_params & params, const std::string & value) {
                    params.speculative.draft.cache_type_v = kv_cache_type_from_str(value);
                })
                .set_env("LLAMA_ARG_SPEC_DRAFT_CACHE_TYPE_V"));
    add_opt(common_arg({ "--spec-draft-override-tensor", "-otd", "--override-tensor-draft" },
                       "<tensor name pattern>=<buffer type>,...", "override tensor buffer type for draft model",
                       [](common_params & params, const std::string & value) {
                           parse_tensor_buffer_overrides(value, params.speculative.draft.tensor_buft_overrides);
                       })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-draft-cpu-moe", "-cmoed", "--cpu-moe-draft" },
    ...
    add_opt(common_arg({ "--spec-draft-n-cpu-moe", "--spec-draft-ncmoe", "-ncmoed", "--n-cpu-moe-draft" }, "N",
    ...
    add_opt(common_arg({ "--spec-draft-p-min", "--draft-p-min" }, "P",
                       string_format("minimum speculative decoding probability (greedy) (default: %.2f)",
                                     (double) params.speculative.draft.p_min),
                       [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg({ "--spec-draft-backend-sampling" }, { "--no-spec-draft-backend-sampling" },
                       string_format("offload draft sampling to the backend (default: %s)",
                                     params.speculative.draft.backend_sampling ? "enabled" : "disabled"),
                       [](common_params & params, bool value) { params.speculative.draft.backend_sampling = value; })
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI })
                .set_env("LLAMA_ARG_SPEC_DRAFT_BACKEND_SAMPLING"));
    add_opt(common_arg({ "--spec-draft-device", "-devd", "--device-draft" }, "<dev1,dev2,..>",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    GGML_ASSERT(params.speculative.draft.n_gpu_layers <
                0);  // string_format would need to be extended for a default >= 0
    add_opt(
        common_arg({ "--spec-draft-ngl", "-ngld", "--gpu-layers-draft", "--n-gpu-layers-draft" }, "N",
    ...
    add_opt(common_arg({ "--spec-draft-model", "-md", "--model-draft" }, "FNAME",
    ...
    add_opt(common_arg({ "--spec-type" }, common_speculative_all_types_str(),
    ...
    add_opt(common_arg({ "--spec-ngram-mod-n-min" }, "N",
                       string_format(
                           "minimum number of ngram tokens to use for ngram-based speculative decoding (default: %d)",
                           params.speculative.ngram_mod.n_min),
                       [](common_params & params, int value) {
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-mod-n-max" }, "N",
                       string_format(
                           "maximum number of ngram tokens to use for ngram-based speculative decoding (default: %d)",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-mod-n-match" }, "N",
    ...
    add_opt(common_arg({ "--spec-ngram-simple-size-n" }, "N",
                       string_format(
                           "ngram size N for ngram-simple speculative decoding, length of lookup n-gram (default: %d)",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-simple-size-m" }, "N",
                       string_format(
                           "ngram size M for ngram-simple speculative decoding, length of draft m-gram (default: %d)",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-simple-min-hits" }, "N",
                       string_format("minimum hits for ngram-simple speculative decoding (default: %d)",
                                     params.speculative.ngram_simple.min_hits),
                       [](common_params & params, int value) {
                           if (value < 1) {
    ...
    add_opt(common_arg({ "--spec-ngram-map-k-size-n" }, "N",
                       string_format(
                           "ngram size N for ngram-map-k speculative decoding, length of lookup n-gram (default: %d)",
                           params.speculative.ngram_map_k.size_n),
                       [](common_params & params, int value) {
                           if (value < 1 || value > 1024) {
                               throw std::invalid_argument("ngram size N must be between 1 and 1024 inclusive");
                           }
    ...
    add_opt(common_arg(
                ...
                string_format("ngram size M for ngram-map-k speculative decoding, length of draft m-gram (default: %d)",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-map-k-min-hits" }, "N",
                       string_format("minimum hits for ngram-map-k speculative decoding (default: %d)",
                                     params.speculative.ngram_map_k.min_hits),
                       [](common_params & params, int value) {
                           if (value < 1) {
                               throw std::invalid_argument("ngram min hits must be at least 1");
                           }
    ...
    add_opt(common_arg({ "--spec-ngram-map-k4v-size-n" }, "N",
                       string_format(
                           "ngram size N for ngram-map-k4v speculative decoding, length of lookup n-gram (default: %d)",
                           params.speculative.ngram_map_k4v.size_n),
                       [](common_params & params, int value) {
                           if (value < 1 || value > 1024) {
                               throw std::invalid_argument("ngram size N must be between 1 and 1024 inclusive");
                           }
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-map-k4v-size-m" }, "N",
                       string_format(
                           "ngram size M for ngram-map-k4v speculative decoding, length of draft m-gram (default: %d)",
                ...
                .set_spec()
                .set_examples({ LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI }));
    add_opt(common_arg({ "--spec-ngram-map-k4v-min-hits" }, "N",
                       string_format("minimum hits for ngram-map-k4v speculative decoding (default: %d)",
                                     params.speculative.ngram_map_k4v.min_hits),
                       [](common_params & params, int value) {
                           if (value < 1) {
                               throw std::invalid_argument("ngram min hits must be at least 1");
                           }
    ...
    add_opt(
        common_arg({ "--draft", "--draft-n", "--draft-max" }, "N",
    ...
    add_opt(
        common_arg(
            ...
            [](common_params & /*params*/, int /*value*/) { arg_removed("use the respective --spec-ngram-*-size-n"); })
            .set_spec()
            .set_examples({ LLAMA_EXAMPLE_SERVER }));
    add_opt(
        common_arg(
            { "--spec-ngram-size-m" }, "N", "the argument has been removed. use the respective --spec-ngram-*-size-m",
            [](common_params & /*params*/, int /*value*/) { arg_removed("use the respective --spec-ngram-*-size-m"); })
            .set_spec()
            .set_examples({ LLAMA_EXAMPLE_SERVER }));
    ...
    add_opt(common_arg(
                ...
                string_format("diffusion algorithm: 0=DIFFUSION_ALGORITHM_ORIGIN, 1=DIFFUSION_ALGORITHM_ENTROPY_BASED, "
    ...
    add_opt(
        common_arg({ "--check" },
                   ...
                   [](common_params & params) { params.check = true; })
    ...
    add_opt(common_arg({ "--spec-default" }, string_format("enable default speculative decoding config"),
                       [](common_params & params) {
